...
The value is concentrated in four non-local invariants: the static/dynamic
parameter split, the parameter-override ordering, extension registration, and
...
`vendorDir`, `rootDir`, `debugMode`, `productionMode` — the legacy inverse of
`debugMode`, kept in sync by `setDebugMode` — `consoleMode`, `baseUrl`) into **both**
...
`appDir` and `wwwDir` are derived from `debug_backtrace`: `appDir` is the
directory of the file that instantiated the `Configurator`, `wwwDir` that of the
outermost running script. Moving `new Configurator` into a factory or helper file
silently changes `%appDir%`.
...
2. the user's configs (file paths or inline arrays) are loaded;
...
Equally important is what the key does **not** contain: the *contents* of config
files (only their paths / inline arrays are in the key) and none of the
code-level customizations — `defaultExtensions`, `onCompile`,
`autowireExcludedClasses`, `addServices`. Editing a NEON file or adding an
extension to the map never changes the key. Such changes are picked up only in
debug mode: `loadContainer` passes `debugMode` as `ContainerLoader`'s
`autoRebuild` flag, and the loader then re-checks tracked dependencies (nette/di's
`DependencyChecker`). In production a container with an existing key is **never
re-checked** — deploying changed configs requires purging the cache.
...
- The map is a plain public property; `createRobotLoader` mutates the
  ...
  `ApplicationExtension` — so it takes effect only when called **before**
  `createContainer()`/`loadContainer()`.
...
| Bundled extensions (`constants`, `php` config sections) | `src/Bootstrap/Extensions/` — `ConstantsExtension`, `PhpExtension` |
| Deprecated alias `Nette\Configurator` | `src/Configurator.php` (`class_alias`) |
